...
export function DateFilterColumn({
  ...
  const allFilter = dateFilters.find((f) => f.type === "all");
        ...
        {allFilter && (
          <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
            <DateFilterItem
              filter={allFilter}
              index={0}
              isSelected={selectedDateFilter?.type === "all"}
        ...
        <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
          {todayFilter && (
            <DateFilterItem
              ...
              isFocused={focusedDateIndex === (allFilter ? 1 : 0)}
              onDateFilterClick={onDateFilterClick}
              onDateFilterDrop={onDateFilterDrop}
            />
          )}
          {tomorrowFilter && (
            <DateFilterItem
              ...
              index={(allFilter ? 1 : 0) + (todayFilter ? 1 : 0)}
              isSelected={selectedDateFilter?.type === "tomorrow"}
              isFocused={
                focusedDateIndex === (allFilter ? 1 : 0) + (todayFilter ? 1 : 0)
              }
              onDateFilterClick={onDateFilterClick}
              onDateFilterDrop={onDateFilterDrop}
            />
